...
All input values must be known for Terraform to run the checks during planning. If an input depends on a resource being created in the same operation, Terraform may defer the data source read until apply.

The check fails when the cluster is outside the requested zone, a host lacks the requested NIC or bonding, VTEP address selection is missing or ambiguous, a VTEP address is duplicated, any non-self directed address pair is missing or not `Connected`, or the requested VNI range overlaps another range in the same pool.
